// App.tsx - WITH TIGER AVATAR CHANGING ON PROGRESS! 🐯✨
...
function TraceLetterA() {
  ...
  return (
    <>
      {/* 🐯 TIGER AVATAR - Now OUTSIDE canvas! */}
      <AnimatedTigerAvatar 
       allDone={allDone} 
    ...
    <View style={styles.canvasWrap}>
      ...
      <Svg width={CANVAS_W} height={CANVAS_H}>
        ...
        <AnimatedGuideDots segment={segments[segIndex]} visible={!allDone} />
      </Svg>
...
// 🐯 TIGER AVATAR - Changes based on progress!
function AnimatedTigerAvatar({ 
  ...
  const getTigerImage = () => {
    if (allDone) {
      return TIGER_SURPRISED; // Excited when complete!
    } else if (currentSegment === 0) {
      return TIGER_WAVING; // Waving at start
    } else {
      return TIGER_HAPPY; // Happy during tracing
    }
  };
  ...
  const celebrateRotate = celebrate.interpolate({
    inputRange: [-1, 1],
      ...
      style={[
        styles.tigerContainer,
        {
          transform: [
            { translateY },
            { scale },
            { rotate: allDone ? celebrateRotate : '0deg' },
          ],
        },
      ]}
    >
      <Image
        source={getTigerImage()}
...
function AnimatedHeaderHUD({ currentSegment }: { currentSegment: any }) {
  ...
  return (
    <View style={styles.hud}>
      ...
      <TouchableOpacity style={styles.closeBtn} activeOpacity={0.7} onPress={() => {}}>
...
function SuccessBadge({ onRestart }: { onRestart: () => void }) {
  ...
  return (
    <>
      <View style={StyleSheet.absoluteFill} pointerEvents="none">
        {confetti.map((conf) => (
          <ConfettiPiece key={conf.id} x={conf.x} color={conf.color} delay={conf.id * 50} />
        ))}
      </View>
...
function SegmentTracer({ segment, onDone }: { segment: any; onDone: () => void }) {
  ...
  const setPuckToIndex = useCallback((idx: number) => {
      ...
      <Svg width={CANVAS_W} height={CANVAS_H} style={StyleSheet.absoluteFill}>
        ...
        <Path
          d={
            ...
            segment.pts
              .slice(1, progressIndexRef.current + 1)
              .map((p: any) => `L ${p.x} ${p.y}`)
              .join(' ')
          }
...
const styles = StyleSheet.create({
  ...
  overlay: { flex: 1, alignItems: 'center', justifyContent: 'center', paddingTop: 20, paddingBottom: 28 },
  canvasWrap: { width: CANVAS_W, height: CANVAS_H, alignItems: 'center', justifyContent: 'center' },
  
  // TIGER AVATAR - Left side spectator
  tigerContainer: {
    position: 'absolute',
    bottom: 80,
    left: 30,
  ...
  tigerImage: {
    width: 280,    // Changed from 120 to 180 (50% bigger!)
    height: 280,   // Changed from 120 to 180 (50% bigger!)
  },
  speechBubble: {
    position: 'absolute',
    top: -40,      // Adjusted for bigger tiger
    left: 140,     // Adjusted for bigger tiger
  ...
  progressBarContainer: {
    position: 'absolute',
    top: 37,         // Changed from 48 to 55 (moved up 7px)
    flexDirection: 'row',
    alignItems: 'center',
  ...
  hud: { position: 'absolute', top: 8, width: CANVAS_W, paddingHorizontal: 12, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  closeBtn: { width: 38, height: 38, borderRadius: 19, backgroundColor: 'rgba(0,0,0,0.5)', alignItems: 'center', justifyContent: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.3, shadowRadius: 3, elevation: 4 },
  closeText: { color: 'white', fontSize: 24, lineHeight: 26, marginTop: -2 },
  tip: { paddingHorizontal: 16, paddingVertical: 10, borderRadius: 20, backgroundColor: 'rgba(255,255,255,0.98)', shadowColor: '#FFD700', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.5, shadowRadius: 8, elevation: 5 },
  tipText: { color: '#0F172A', fontWeight: '800', fontSize: 13 },
  legend: { position: 'absolute', bottom: 8, alignSelf: 'center', flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(255,255,255,0.95)', paddingHorizontal: 14, paddingVertical: 10, borderRadius: 20, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.2, shadowRadius: 5, elevation: 5 },
  ...
  puck: { position: 'absolute', width: 40, height: 40, borderRadius: 20, backgroundColor: '#FFD700', alignItems: 'center', justifyContent: 'center', shadowColor: '#FFD700', shadowOpacity: 0.8, shadowRadius: 10, shadowOffset: { width: 0, height: 4 }, elevation: 8, borderWidth: 3, borderColor: '#FFA500' },
  ...
  puckArrow: { color: 'white', fontWeight: '900', fontSize: 22, textShadowColor: 'rgba(0,0,0,0.5)', textShadowOffset: { width: 1, height: 1 }, textShadowRadius: 2 },
  success: { position: 'absolute', top: CANVAS_H / 3.5, alignSelf: 'center', paddingHorizontal: 32, paddingVertical: 24, borderRadius: 28, backgroundColor: 'rgba(34,197,94,0.98)', alignItems: 'center', shadowColor: '#22C55E', shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.5, shadowRadius: 12, elevation: 12, borderWidth: 4, borderColor: '#FFF' },
  successEmoji: { fontSize: 64, marginBottom: 12 },
  successText: { color: 'white', fontSize: 32, fontWeight: '900', textAlign: 'center', textShadowColor: 'rgba(0,0,0,0.3)', textShadowOffset: { width: 2, height: 2 }, textShadowRadius: 4 },
  successSub: { color: 'white', fontSize: 16, textAlign: 'center', marginTop: 6, opacity: 0.95, fontWeight: '700' },
  restartBtn: { backgroundColor: 'rgba(255,255,255,0.3)', paddingHorizontal: 24, paddingVertical: 12, borderRadius: 24, marginTop: 16, borderWidth: 2, borderColor: 'rgba(255,255,255,0.5)' },
