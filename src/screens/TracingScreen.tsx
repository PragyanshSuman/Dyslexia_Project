...
// Letter tracing with Tiger + Bee glitter trail, routed by letter param.
// Changes vs previous:
// - Reads `letter` from route and chooses geometry via useGeometryByLetter
// - Close button navigates back
// - HUD shows current letter
...
// Placeholder switch (extend with B..Z)
function useGeometryByLetter(letter: string) {
  switch (letter) {
    case 'A':
    default:
      return useALetterGeometry();
  }
}
...
export default function TracingScreen() {
  ...
  // Get geometry per letter
  const { segments } = useGeometryByLetter(letter);
...
// Main tracing component (now accepts letter + segments)
function TraceLetter({
  ...
  return (
    <>
      ...
      {/* Full-width HUD for centered tip + current letter */}
      ...
      <TouchableOpacity style={styles.closeBtnScreen} activeOpacity={0.7} onPress={onClose}>
      ...
      <View style={styles.canvasWrap}>
        ...
        {!allDone && !showBeeAnimation ? (
          <SegmentTracer key={`${letter}-${segIndex}`} segment={segments[segIndex]} onDone={goNext} />
        ) : !showBeeAnimation ? (
